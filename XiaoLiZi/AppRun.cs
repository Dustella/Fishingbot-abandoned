...
namespace RobotCore
{
    public static class AppRun
    {
        ...
        private static string SetProperty(string json, AppInfo appInfo)
        {
            ...
            json = appInfo.SetPermission(134, "添加好友_取验证类型", json);
            ...
            json = appInfo.SetPermission(138, "红包转发", json);
            json = appInfo.SetPermission(139, "发送数据包", json);
            json = appInfo.SetPermission(140, "请求ssoseq", json);
            json = appInfo.SetPermission(141, "取sessionkey", json);
            json = appInfo.SetPermission(142, "获取bkn_gtk", json);
            json = appInfo.SetPermission(143, "置好友验证方式", json);
            json = appInfo.SetPermission(144, "上传照片墙图片", json);
            json = appInfo.SetPermission(145, "付款", json);
            return json;
        }
    }
}
